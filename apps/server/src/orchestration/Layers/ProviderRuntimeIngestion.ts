    ...
    Cache.getOption(bufferedAssistantTextByMessageId, messageId).pipe(
      Effect.flatMap((existingText) =>
        Effect.gen(function* () {
          ...
          // Thinking summaries often arrive as sentences without blank lines.
          // Show the first readable fragment, then pace incremental chunks;
          // never hold the whole trace until a tool or the answer begins.
          if (
            mode === "paragraph" &&
            messageStreamRoleOf(messageId) === "reasoning" &&
            ready.length === 0 &&
            paced &&
            nextText.length >= (lastDeliveredAt === undefined ? 16 : 96)
          ) {
            ready = nextText;
            rest = "";
          }
    ...
    Effect.gen(function* () {
      ...
      if (isTerminalTurn) {
        ...
        if (turnId) {
          ...
          yield* clearAssistantSegmentStateForTurn(thread.id, turnId);
          // Keep the closed segment identity in the bounded turn cache so a
          // late native completion cannot backfill the same trace twice.
          const reasoningState = yield* getAssistantSegmentStateForTurn(
            thread.id,
            turnId,
            "reasoning",
          );
          if (Option.isSome(reasoningState)) {
            yield* setAssistantSegmentStateForTurn(
              thread.id,
              turnId,
              {
                ...reasoningState.value,
