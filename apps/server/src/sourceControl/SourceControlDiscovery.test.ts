  ...
  () =>
    Effect.gen(function* () {
      for (const scenario of [
        ...
        const commands: string[] = [];
        const useTea = scenario === "missing" || scenario === "absent";
        yield* Effect.gen(function* () {
          ...
          assert.strictEqual(result.executable, useTea ? "tea" : "fj");
          assert.strictEqual(
            result.auth.status,
            scenario === "revoked"
              ? "unauthenticated"
          ...
          assert.deepStrictEqual(
            result.auth.account,
            scenario === "authenticated" || useTea ? Option.some("maria") : Option.none(),
          );
          assert.strictEqual(
            commands.some((command) => command.startsWith("tea ")),
            useTea,
          );
          assert.strictEqual(commands.includes("fj version"), scenario !== "absent");
        }).pipe(
          Effect.provideService(CommandAvailability, (command) =>
            Effect.succeed(command !== "fj" || scenario !== "absent"),
          ),
          Effect.provide(
            Layer.mergeAll(
              NodeServices.layer,
