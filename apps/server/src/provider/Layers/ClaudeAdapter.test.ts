...
class FakeClaudeQuery implements AsyncIterable<SDKMessage> {
  ...
  /** The real CLI answers interrupt() with an aborted result; tests can turn that off. */
  public interruptEmitsResult = true;
  ...
  readonly interrupt = async (): Promise<void> => {
    ...
    if (this.interruptEmitsResult) {
      this.emit({
        ...
        uuid: `interrupt-result-${this.interruptCalls.length}`,
...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("forwards declared custom-model capabilities into query options", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.equal(capableOptions?.thinking, undefined);
      assert.equal(capableOptions?.extraArgs?.["thinking-display"], undefined);
  ...
  for (const model of ["k3", "glm-5.3", "oai-gpt-6-astra"]) {
    it.effect(`preserves the thinking protocol of custom route ${model}`, () => {
      const harness = makeHarness({ claudeConfig: { customModels: [model] } });
      return Effect.gen(function* () {
        const adapter = yield* ClaudeAdapter;
        yield* adapter.startSession({
          threadId: THREAD_ID,
        ...
        assert.equal(options?.extraArgs?.["thinking-display"], undefined);
      }).pipe(
        Effect.provideService(Random.Random, makeDeterministicRandomService()),
        Effect.provide(harness.layer),
      );
  ...
  it.effect("keeps a user-supplied --thinking-display over the Mythos default", () => {
    const harness = makeHarness({
  ...
  it.effect("requests summaries for standard Claude models", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      yield* adapter.startSession({
  ...
  it.effect("forwards thinking deltas once without legacy reasoning activities", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      ...
      assert.ok(reasoningDeltas.every((event) => event.turnId === turn.turnId));
      assert.equal(
        runtimeEvents.some(
          (event) =>
    ...
    it.effect(
      ...
      () => {
        ...
        return Effect.gen(function* () {
          ...
          assert.equal(reasoningCompleted?.type, "content.delta");
          if (reasoningCompleted?.type === "content.delta") {
            assert.equal(reasoningCompleted.payload.delta, "Background thought");
          }
        }).pipe(
          Effect.provideService(Random.Random, makeDeterministicRandomService()),
          Effect.provide(harness.layer),
  ...
  it.effect("ignores a subagent's content_block_stop for the parent's reasoning block", () => {
    ...
    return Effect.gen(function* () {
      ...
      // Subagent block stops do not create a second copy of parent thinking.
  ...
  it.effect("interruptTurn settles live tasks and keeps the provider session", () => {
    ...
    return Effect.gen(function* () {
      ...
      // Cooperative stop: stop live tasks, interrupt the parent turn, keep
  ...
  it.effect(
    "interruptTurn lets the SDK's aborted result close the turn and reuses the query",
    () => {
      const harness = makeHarness();
      return Effect.gen(function* () {
        const adapter = yield* ClaudeAdapter;
        ...
        assert.equal(String(completed?.turnId), String(firstTurn.turnId));
        assert.equal(harness.query.interruptCalls.length, 1);
        assert.equal(harness.query.closeCalls, 0);
        assert.equal(harness.getCreateQueryCalls(), 1);
        assert.equal(yield* adapter.hasSession(session.threadId), true);
  ...
  it.effect("interruptTurn closes the turn locally when the SDK never answers", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      ...
      // The fallback waits on the test clock; advance it until the interrupt settles.
  ...
  it.effect("interruptTurn ignores a Stop aimed at a turn that already ended", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      const session = yield* adapter.startSession({
      ...
      // A delayed Stop for the first turn must not interrupt the second one.
  ...
  it.effect("interruptTurn cancels a pending approval instead of leaving it open", () => {
    const harness = makeHarness();
    return Effect.gen(function* () {
      const adapter = yield* ClaudeAdapter;
      ...
      const permissionPromise = canUseTool(
        ...
        { command: "pwd" },
        {
          ...
          suggestions: [],
          requestId: "stop-pending-approval",
          toolUseID: "tool-use-stop",
        },
      );
      const requested = yield* Stream.runHead(adapter.streamEvents);
      assert.equal(requested._tag === "Some" && requested.value.type, "request.opened");

      // Cancellation resolves the request twice on the wire (the stop path
      // and the canUseTool continuation both emit request.resolved with the
      // same requestId), exactly as the hard stop always did; the turn must
      // complete only after both.
      const settledFiber = yield* adapter.streamEvents.pipe(
        Stream.filter(
          (event) => event.type === "request.resolved" || event.type === "turn.completed",
        ),
        Stream.take(3),
        Stream.runCollect,
        Effect.forkChild,
      ...
      assert.deepEqual(
        ...
        ["request.resolved", "request.resolved", "turn.completed"],
      );
      for (const resolved of settled.slice(0, 2)) {
        assert.equal(resolved.type === "request.resolved" && resolved.payload.decision, "cancel");
      }
      const permissionResult = yield* Effect.promise(() => permissionPromise);
      assert.notEqual((permissionResult as PermissionResult).behavior, "allow");
      assert.equal(harness.query.closeCalls, 0);
      assert.equal(yield* adapter.hasSession(session.threadId), true);
    }).pipe(
      Effect.provideService(Random.Random, makeDeterministicRandomService()),
  ...
  it.effect("consumes undeclared and UX-internal system subtypes without warning rows", () => {
    ...
    return Effect.gen(function* () {
      ...
      // The fallback, notification, informational note, refusal, and retry storm
  ...
  const observeUsageLimitEvents = (adapter: ClaudeAdapterShape, query: FakeClaudeQuery) =>
    Effect.gen(function* () {
      ...
      const runtimeEventsFiber = yield* Stream.runForEach(adapter.streamEvents, (event) =>
        Effect.gen(function* () {
          ...
          if (
            receipt &&
            event.type === "session.state.changed" &&
            event.payload.reason === "session_state:running"
          ) {
      ...
      const drainSdkMessages = Effect.gen(function* () {
        ...
        // This state receipt follows queued SDK messages without injecting a retry warning.
        query.emit({
          type: "system",
          subtype: "session_state_changed",
          state: "running",
