...
function resultOutcome(
  ...
  // Preserve explicit CLI errors and use earlier authentication or usage
  // evidence when the terminal result only reports a generic failure.
  ...
  const resultText = successTaggedFailure ? trimmedString(result.result) : undefined;
  const errorMessage = resultText || listedError || structuredError;
  ...
  if (result.subtype === "success") {
    return { status: result.is_error === true ? "failed" : "completed", errorMessage };
  }
  return {
...
function shouldRequestClaudeThinkingSummaries(input: {
  ...
  // Custom Claude-compatible routes choose their own thinking protocol. Only
  // opt them into adaptive summaries when the user explicitly requests it.
  return (
    input.thinking !== false &&
    input.thinkingDisplay !== "omitted" &&
...
export const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const handleStreamEvent = Effect.fn("handleStreamEvent")(function* (
    ...
    if (event.type === "message_start") {
      ...
      if (context.turnState && !streamParentToolUseId) {
    ...
    if (event.type === "content_block_stop") {
      ...
      if (assistantBlock !== undefined && streamParentToolUseId != null) {
        return;
      }
      if (assistantBlock) {
  ...
  const startSyntheticTurn = Effect.fn("startSyntheticTurn")(function* (
    ...
    context.turnStartMessageIds.push(startMessageId);
    context.turnState = {
      turnId,
      startedAt,
      synthetic: true,
