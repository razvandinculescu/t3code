...
interface TimelineRowSharedState {
  ...
  reasoningExpansionOverrides: ReadonlyMap<string, boolean>;
...
export const MessagesTimeline = memo(function MessagesTimeline({
  ...
  >(() => rememberedPosition?.disclosures?.reasoningMessages ?? new Map());
  ...
  let paintedReasoningExpansionOverrides = reasoningExpansionOverrides;
  if (listIdentityRef.current !== listIdentityKey) {
    ...
    paintedReasoningExpansionOverrides =
      rememberedPosition?.disclosures?.reasoningMessages ?? new Map();
    ...
    setReasoningExpansionOverrides(paintedReasoningExpansionOverrides);
  }
  ...
  const onToggleReasoning = useCallback(
    (messageId: string, expanded: boolean, anchorKey: string) => {
      ...
      setReasoningExpansionOverrides((current) => {
        if (current.get(messageId) === expanded) return current;
      ...
      if (row && element) {
        rememberTimelinePosition(listIdentityKey, {
          ...
          disclosures: {
            ...
            reasoningMessages: paintedReasoningExpansionOverrides,
  ...
  const sharedState = useMemo<TimelineRowSharedState>(
    () => ({
      ...
      onToggleSpawnRow,
      onToggleReasoning,
      reasoningExpansionOverrides: paintedReasoningExpansionOverrides,
      expandReasoningByDefault: reasoningExpandedByDefault || workLogExpandedByDefault,
      workGroupViewState,
  ...
  const getItemType = useCallback(
    (item: MessagesTimelineRow) =>
      resolveTimelineRowItemType(item, {
        workLogExpandedByDefault,
        reasoningExpandedByDefault,
        reasoningExpansionOverrides: paintedReasoningExpansionOverrides,
      }),
    [reasoningExpandedByDefault, workLogExpandedByDefault, paintedReasoningExpansionOverrides],
  );
...
function ReasoningTraceBlock({
  ...
  const expanded =
    !showHeader || (ctx.reasoningExpansionOverrides.get(first.id) ?? ctx.expandReasoningByDefault);
  const streaming =
    live &&
  ...
  return (
    <div className="flex flex-col">
      ...
      {expanded ? (
        <div className="ms-7 flex flex-col gap-3 rounded-md bg-muted/40 px-3 py-2 text-secondary-label select-text">
...
/**
 * A provider's thinking trace. Disclosure overrides live on the list so they
...
const ReasoningTimelineRow = memo(function ReasoningTimelineRow({
  ...
  const { isWorking, unsettledTurnId } = use(TimelineRowActivityCtx);
  const streaming =
    message.streaming && isWorking && message.turnId !== null && message.turnId === unsettledTurnId;
  const expanded = ctx.reasoningExpansionOverrides.get(message.id) ?? ctx.expandReasoningByDefault;
  ...
  return (
    <div className={cn("flex flex-col", expanded && "mb-1")}>
      ...
      {expanded ? (
        <div className="mt-1 ms-7 flex flex-col gap-3 rounded-md bg-muted/40 px-3 py-2 text-secondary-label select-text">
          <ChatMarkdown
            text={message.text}
...
const WorkGroupSection = memo(function WorkGroupSection({
  ...
  // Expanded work logs render all entries without a nested scroller.
...
const workEntryExpandedBodyClassName =
  "cursor-text whitespace-pre-wrap break-words font-mono text-secondary-label text-(length:--font-size-code,var(--text-2xs)) leading-relaxed select-text";
  ...
  return (
    ...
    >
      ...
      {open ? (
        ...
        >
          <pre className={workEntryExpandedBodyClassName}>{body}</pre>
        </div>
      ) : null}
    </div>
  );
...
const PlainWorkEntryRow = memo(function PlainWorkEntryRow(props: {
  ...
  const [expanded, setExpanded] = useState(
    () => autoExpanded || (groupView?.state.expandedEntries.has(workEntry.id) ?? false),
  );
  // The settings hydrate asynchronously and can change while rows are mounted;
  // keep the row in sync without clobbering per-group manual expansions.
  useEffect(() => {
    setExpanded(autoExpanded || (groupView?.state.expandedEntries.has(workEntry.id) ?? false));
  }, [autoExpanded, groupView, workEntry.id]);
  const toggleExpanded = () => {
    const next = !expanded;
    if (groupView) {
