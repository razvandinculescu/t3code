...
describe("deriveMessagesTimelineRows", () => {
  ...
  it("appends queued messages after the live rows, marking the oldest as next", () => {
    ...
    const options = { workLogExpandedByDefault: true, reasoningExpandedByDefault: true };
    const queuedRow = rows[2]!;
    expect(estimateTimelineRowTextLength(queuedRow, options)).toBe(5);
    expect(resolveTimelineRowItemType(queuedRow, options)).toBe("queued-message:s");
    if (queuedRow.kind !== "queued-message") throw new Error("Expected a queued message");
    expect(
      resolveTimelineRowItemType(
        { ...queuedRow, queuedMessage: { ...queuedRow.queuedMessage, prompt: "x".repeat(5_000) } },
        options,
      ),
    ).toBe("queued-message:l");
  });
  ...
  it("leads the worktree setup card with the working header", () => {
    ...
    const setupRow = withoutMessages[1]!;
    const expanded = { workLogExpandedByDefault: true, reasoningExpandedByDefault: true };
    expect(estimateTimelineRowTextLength(setupRow, expanded)).toBe(0);
    expect(resolveTimelineRowItemType(setupRow, expanded)).toBe("worktree-setup");
  ...
  it("keeps reasoning work entries out of the fold when reasoning expansion is on", () => {
    const timelineEntries = [
      {
        id: "user-entry",
        kind: "message" as const,
      ...
      },
      {
        ...
        entry: {
          ...
          detail: "Let me think about this…",
        },
      },
      {
        id: "work-entry-1",
    ...
    // The reasoning row stays visible outside the fold (its text opens from
    // the row's own reasoning setting); other work still folds, anchored where
    // the first hidden entry sits.
    ...
    expect(rows[1]).toMatchObject({ kind: "work", isExpandedToolGroup: false });
  });
  ...
  it.each(["reasoningExpandedByDefault", "workLogExpandedByDefault"] as const)(
    "keeps live thinking visible outside the activity summary with %s",
    (setting) => {
      const thought = reasoningEntry("visible-thought", "2026-01-01T00:00:01Z", "turn-1");
      thought.message.streaming = true;
      const rows = deriveMessagesTimelineRows({
        timelineEntries: [thought, toolEntry("tool", "2026-01-01T00:00:02Z", "turn-1")],
        [setting]: true,
        runningTurnId: TurnId.make("turn-1"),
        isWorking: true,
        activeTurnStartedAt: "2026-01-01T00:00:00Z",
      ...
      expect(rows).toContainEqual(
        expect.objectContaining({ kind: "message", message: thought.message }),
      );
      expect(rows.some((row) => row.kind === "activity-group")).toBe(false);
    },
  );
  ...
  it("expands the live work group by default when the expand settings ask for it", () => {
    ...
    const initial = deriveMessagesTimelineRowsWithState({ timelineEntries, ...baseInput });
    const collapsed = initial.rows;
    ...
    for (const option of ["reasoningExpandedByDefault", "workLogExpandedByDefault"] as const) {
      const expandedInput = { timelineEntries, ...baseInput, [option]: true };
      const expanded = deriveMessagesTimelineRowsWithState(expandedInput, initial);
      expect(expanded.rows.find((row) => row.kind === "work-live")).toMatchObject({
        expanded: true,
      });
      const collapsedAgain = deriveMessagesTimelineRowsWithState(
        { ...expandedInput, [option]: false },
        expanded,
      );
      expect(collapsedAgain.rows.find((row) => row.kind === "work-live")).toMatchObject({
        expanded: false,
      });
    }
  });
  ...
  it("expands settled work groups when work log expansion is on by default", () => {
    const turnId = TurnId.make("turn-worklog-setting");
    ...
    expect(rows.find((row) => row.kind === "work-toggle")).toMatchObject({ expanded: true });
    expect(rows.some((row) => row.kind === "work")).toBe(true);
  });

  it("expands groups containing reasoning when reasoning expansion is on by default", () => {
    // Reasoning stays out of the settled-turn fold, so a settled group that
    // contains reasoning is one of consecutive thinking blocks.
    const turnId = TurnId.make("turn-reasoning-setting");
    const timelineEntries = [0, 1].map((index) => ({
      id: `work-entry-${index}`,
      kind: "work" as const,
      ...
      entry: {
        id: `work-${index}`,
        createdAt: `2026-01-01T00:00:0${index + 1}Z`,
        toolCallId: `reasoning-${index}`,
        turnId,
        label: "Reasoning",
        detail: index === 0 ? "Thinking about the fix" : "Checking the edge cases",
        tone: "tool" as const,
        itemType: "reasoning" as const,
        toolLifecycleStatus: "completed" as const,
      },
...
describe("timeline row size buckets", () => {
  ...
  it("sizes expanded activity history using visible reasoning disclosures", () => {
    const message: ChatMessage = {
      id: MessageId.make("grouped-thought"),
      role: "reasoning",
      text: "x".repeat(9000),
    ...
    const row: MessagesTimelineRow = {
      ...
      expanded: true,
      active: false,
      entries: [{ kind: "message", id: message.id, createdAt: message.createdAt, message }],
    };
    expect(resolveTimelineRowItemType(row, collapsed)).toBe("activity-group:s");
    expect(
      resolveTimelineRowItemType(row, {
        ...collapsed,
