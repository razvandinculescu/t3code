...
describe("deriveMessagesTimelineRows", () => {
  ...
  it("keeps reasoning work entries out of the fold when reasoning expansion is on", () => {
    ...
    // The reasoning row stays visible outside the fold; other work still
    // folds, anchored where the first hidden entry sits.
  ...
  it("renders every work row and no toggle when work log expansion is on by default", () => {
    const timelineEntries = [
      {
        id: "work-entry-1",
        kind: "work" as const,
    ...
    expect(rows.map((row) => row.id)).toEqual(["work-1", "work-2", "work-3"]);
