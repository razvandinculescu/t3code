...
describe("reasoning outcomes shared by web and mobile", () => {
  it.each(["inProgress", "completed", "failed"])(
    "keeps %s reasoning out of tool outcomes",
    (status) => {
      const entry = {
        label: "Reasoning",
        tone: "tool" as const,
        itemType: "reasoning" as const,
        detail: "The previous attempt returned exit code 1: command not found",
