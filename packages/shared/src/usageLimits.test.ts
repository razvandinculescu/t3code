...
describe("pooled cached wrapper limits", () => {
  it("keeps wrapper identity and replaces expired percentages with a notice", () => {
    const checkedAt = "2026-09-03T11:00:00.000Z";
    const input = new Map([
      [
        EnvironmentId.make("env-a"),
        {
          ...
          serverConfig: {
            providers: [
              provider({
                instanceId: ProviderInstanceId.make("claude-k3"),
    ...
    const expiresAt = Date.parse(checkedAt) + 15 * 60_000;
    expect(collectLimitAccounts(input, expiresAt)[0]).toMatchObject({
      displayName: "Kimi K3",
      accentColor: "#123456",
      redeem: { environmentId: "env-a", input: { instanceId: "claude-k3" } },
    });
    expect(collectLimitAccounts(input, expiresAt + 1)).toEqual([]);
    expect(collectLimitNotices(input, expiresAt + 1)).toEqual([
      "Kimi K3: Limits are out of date. Refresh to try again.",
    ]);
  });

  it("excludes expired hub accounts and explains why their bars disappeared", () => {
    const checkedAt = "2026-09-03T11:00:00.000Z";
    const input = new Map([
      [
        EnvironmentId.make("env-a"),
    ...
    expect(collectLimitAccounts(input, now)).toEqual([]);
    expect(collectLimitNotices(input, now)).toEqual([
      "Hub: Limits are out of date. Refresh to try again.",
    ]);
  });
});
