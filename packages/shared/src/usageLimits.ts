  ...
  for (const [environmentId, presentation] of presentations) {
    ...
    for (const provider of providersWithLimits(presentation.serverConfig?.providers ?? [])) {
      if (
        ...
        limitsNotice(
          provider.usageLimits,
          now === undefined ? undefined : Math.max(now, Date.parse(provider.checkedAt)),
        ) !== null
      )
        continue;
      merge(
  ...
  for (const presentation of presentations.values()) {
    for (const source of presentation.serverConfig?.usageLimitSources ?? []) {
      ...
      for (const account of source.accounts) {
        if (limitsNotice(account.usageLimits, now) !== null) continue;
    ...
    for (const source of presentation.serverConfig?.usageLimitSources ?? []) {
      ...
      if (
        ...
        source.accounts.some(
          (account) =>
            account.usageLimits.windows.length > 0 &&
            !account.usageLimits.unavailable &&
            limitsNotice(account.usageLimits, now) !== null,
        )
      ) {
        notices.push(
          `${label(environmentLabel, source.label)}: Limits are out of date. Refresh to try again.`,
        );
      }
    }
  }
